 ...
 *       'w'       jog +y by the default Y distance (0.5 inch)
 *       's'       jog -y by the default Y distance (0.5 inch)
 *       'a'       jog -x by the default X distance (0.5 inch)
 *       'd'       jog +x by the default X distance (0.5 inch)
 *       '['       jog -x by the full resurfacing amount (61 inches)
 *       ']'       jog +x by the full resurfacing amount (61 inches)
 *       'u'       jog +y by the resurfacing amount (0.8 inches)
 *       'n'       jog -y by the resurfacing amount (0.8 inches)
 *       'i'       jog +z by a thousandth of an inch
 *       'k'       jog -z by a thousandth of an inch
 *       'o'       jog up (+z) by the default Z distance (2mm)
 *       'l'       jog down (-z) by the default Z distance (2mm)
 *       '0'       vacuum off
 *       '1'       vacuum on
 *
...
const int velocityLimit = int(100.0 * motorCountsPerInch / 60.0);      // 100 IPM
const int accelerationLimit = 240000;  // counts per sec^2

// Define jog distances (in pulses)
const int jogDistanceX = int(0.5 * motorCountsPerInch);
const int jogDistanceY = int(0.5 * motorCountsPerInch);
const int jogDistanceZ = int(2.5 * motorCountsPerMillimeter);

const int jogResurfacingX = int(61.0 * motorCountsPerInch);
const int jogResurfacingY = int(0.8 * motorCountsPerInch);
const int jogResurfacingZ = int(0.001 * motorCountsPerInch);
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (!isAlphaNumeric(command) && command != '[' && command != ']') {
...
void processCommand(char command) {
  ...
  switch (command) {
    case '[': // jog -x by the full resurfacing amount
      if (!softStopped) { moveRelativeX(-jogResurfacingX); }
      break;
    case ']': // jog +x by the full resurfacing amount (61 inches)
      if (!softStopped) { moveRelativeX(jogResurfacingX); }
      break;
    case 'u': // jog +y by the resurfacing amount (0.8 inches)
      if (!softStopped) { moveRelativeY(jogResurfacingY); }
      break;
    case 'n': // jog -y by the resurfacing amount (0.8 inches)
      if (!softStopped) {moveRelativeY(-jogResurfacingY); }
      break;
    case 'i': // jog +z by a thousandth of an inch
      if (!softStopped) {moveRelativeZ(jogResurfacingZ); }
      break;
    case 'k': // jog -z by a thousandth of an inch
      if (!softStopped) {moveRelativeZ(-jogResurfacingZ); }
      break;
